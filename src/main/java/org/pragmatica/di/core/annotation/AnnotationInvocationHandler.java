 ...
 */
package org.pragmatica.di.core.annotation;
...
/**
 * An implementation of {@link Annotation} that mimics the behavior of normal annotations. It is an {@link InvocationHandler}, meant to be used via
 * {@link AnnotationFactory#create(Class, Map)}.
 * <p>
 * The constructorSupplier checks that the all the elements required by the annotation interface are provided and that the types are compatible. If
 * extra elements are provided, they are ignored. If a value is of an incompatible type is provided or no value is provided for an element without a
 * default value, {@link InjectorException} is thrown.
 * </p>
 * <p>
 * Note: {@link #equals(Object)} and {@link #hashCode()} and implemented as specified by {@link Annotation}, so instances are safeCreate to mix with
 * normal annotations.
 * <p>
...
class AnnotationInvocationHandler implements Annotation, InvocationHandler, Serializable {
    ...
    private static Class<? extends Annotation> validateType(Class<? extends Annotation> annotationType) {
        var interfaces = annotationType.getInterfaces();
    ...
    private static Map<String, Object> normalize(Class<? extends Annotation> annotationType,
        ...
        for (var method : annotationType.getDeclaredMethods()) {
            var value = calculateValue(source, method);
    ...
    private static Object calculateValue(Map<String, Object> values, Method method) {
        ...
        var value = values.getOrDefault(name, method.getDefaultValue());
    ...
    private static void validateReturnType(Method method, Object value) {
        ...
        if (!returnType.isInstance(value)) {
            throw new InjectorException("Incompatible type "
                                        + value.getClass()
                                        + " provided for '"
                                        + method.getName()
                                        + "()' of "
    ...
    public boolean equals(Object other) {
        ...
        for (var entry : values.entrySet()) {
            try {
                if (!deepEquals(entry.getValue(), that.annotationType().getMethod(entry.getKey()).invoke(that))) {
                    return false;
                }
    ...
    private String calculateToStringValue() {
        var result = new StringBuilder().append('@').append(annotationType.getName()).append('(');

        for (var name : new TreeSet<>(values.keySet())) {
    ...
    private int calculateHashCode(Object element) {
        ...
        return switch (element) {
            case Object[] objects -> Arrays.hashCode(objects);
            case byte[] bytes -> Arrays.hashCode(bytes);
            case short[] shorts -> Arrays.hashCode(shorts);
            case int[] integers -> Arrays.hashCode(integers);
            ...
            default -> Objects.hashCode(element);
        };
    }
}
